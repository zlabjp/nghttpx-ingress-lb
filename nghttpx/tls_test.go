...
func TestAddOrUpdateCertAndKey(t *testing.T) {
	...
	if got, want := tlsCred.Key, filepath.Join(tlsDirectory, fmt.Sprintf("%v.key", name)); got != want {
		t.Errorf("tlsCred.Key = %v, want %v", got, want)
	}
	if got, want := tlsCred.Cert, filepath.Join(tlsDirectory, fmt.Sprintf("%v.crt", name)); got != want {
