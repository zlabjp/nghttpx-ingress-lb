...
func (lbc *LoadBalancerController) createUpstream(ing *networking.Ingress, host, path string, isb *networking.IngressServiceBackend,
	...
	for i := range svc.Spec.Ports {
		...
		var key string
		if isb.Port.Name != "" {
			if isb.Port.Name == servicePort.TargetPort.String() || isb.Port.Name == servicePort.Name {
				key = isb.Port.Name
			}
		} else if isb.Port.Number == int32(servicePort.TargetPort.IntValue()) || isb.Port.Number == servicePort.Port {
